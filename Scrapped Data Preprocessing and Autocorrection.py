        ...
        while page_num <= 200:
            ...
            for i in range(len(authors)):
                if authors[i].text == '\n':
                    ...
                    authorsList.append(authors[i].text.replace("\n", ""))
...
class ScrapData(object):
    ...
    def __init__(self, file_path):
        ...
        for i, link in enumerate(linksList):
            ...
            src = result.content
            soup = BeautifulSoup(src, "lxml")
            details = soup.find("div", {"class":"book-info"}).find("ul")
            det_str = ""
            i = 0
            for li in details.find_all("li"):
                if i == 1:
                    det_str += li.find("a").text
                    det_str = det_str.split("تحميل ")
                    break
                i += 1
            details = re.sub('\n*', '', det_str[1])  
            categoriesList.append(details)     
